...
const Dashboard = () => {
    ...
    return (<div className="min-h-screen pt-20 pb-16">
      <div className="container mx-auto px-4 max-w-3xl">
        ...
        {bookings.length === 0 ? (<div className="text-center py-16 glass-card">
            ...
            {bookings.map((booking, i) => (<motion.div key={booking.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }} className="glass-card overflow-hidden">
                <div className="flex flex-col sm:flex-row">
                  <img src={booking.restaurantImage} alt={booking.restaurantName} className="w-full sm:w-32 h-32 object-cover" loading="lazy" width={128} height={128}/>
                  <div className="p-4 flex-1">
                    <div className="flex items-start justify-between">
                      ...
                      <span className={cn("rounded-full px-2.5 py-1 text-xs font-medium", booking.status === 'confirmed' ? 'bg-secondary/10 text-secondary' :
                    booking.status === 'cancelled' ? 'bg-destructive/10 text-destructive' :
                        'bg-accent/10 text-accent-foreground')}>
                        {booking.status}
                      </span>
                    </div>
                    {booking.status === 'confirmed' && (<button onClick={() => cancelBooking(booking.id)} className="mt-3 flex items-center gap-1 text-xs text-destructive hover:underline">
