...
const RestaurantDetails = () => {
    ...
    return (<div className="min-h-screen pt-0">
      {/* Hero */}
      ...
      <div className="container mx-auto px-4 -mt-20 relative z-10">
          ...
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                {isAuthenticated && (<button onClick={() => toggleFavorite(restaurant.id)} className="p-2 rounded-lg hover:bg-muted transition-colors">
              ...
              <div className="flex flex-wrap gap-2">
                {restaurant.tags.map(tag => (<span key={tag} className="rounded-full bg-muted px-3 py-1 text-xs text-muted-foreground">{tag}</span>))}
              </div>
        ...
        <div className="flex gap-1 border-b border-border mb-6">
          {['menu', 'reviews', 'info'].map(tab => (<button key={tab} onClick={() => setActiveTab(tab)} className={cn("px-4 py-3 text-sm font-medium transition-colors capitalize border-b-2", activeTab === tab ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground")}>
              {tab}
            </button>))}
        </div>
        ...
        <motion.div key={activeTab} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="pb-16">
          {activeTab === 'menu' && (<div className="space-y-8">
              {menuCategories.map(cat => (<div key={cat}>
                  <h3 className="font-display text-xl font-semibold text-foreground mb-4">{cat}</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {restaurant.menu.filter(m => m.category === cat).map(item => (<div key={item.id} className="glass-card p-4 flex justify-between items-start">
                        <div>
          ...
          {activeTab === 'reviews' && (<div className="space-y-4">
              {restaurant.reviews.map(review => (<div key={review.id} className="glass-card p-5">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1">
                        {Array.from({ length: 5 }).map((_, i) => (<Star key={i} className={`h-3.5 w-3.5 ${i < review.rating ? 'fill-accent text-accent' : 'text-border'}`}/>))}
          ...
          {activeTab === 'info' && (<div className="glass-card p-6">
              <h3 className="font-display text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                <Clock className="h-5 w-5 text-primary"/> Opening Hours
              </h3>
              <div className="space-y-2">
                {restaurant.openingHours.map((h, i) => (<div key={i} className="flex justify-between text-sm">
