        ...
        <div className="space-y-4 mb-8">
          ...
          <div className="flex flex-wrap gap-3">
            <div className="flex flex-wrap gap-2">
              {cuisineTypes.map(c => (<button key={c} onClick={() => setCuisine(c)} className={cn("rounded-xl px-3 py-1.5 text-xs font-medium transition-all border", cuisine === c ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-foreground hover:border-primary/50")}>
                  {c}
                </button>))}
            </div>
            <div className="flex flex-wrap gap-2">
              {priceRanges.map(p => (<button key={p} onClick={() => setPrice(p)} className={cn("rounded-xl px-3 py-1.5 text-xs font-medium transition-all border", price === p ? "border-primary bg-primary text-primary-foreground" : "border-border bg-card text-foreground hover:border-primary/50")}>
                  {p}
                </button>))}
            </div>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="rounded-xl border border-border bg-card px-3 py-1.5 text-xs font-medium text-foreground">
